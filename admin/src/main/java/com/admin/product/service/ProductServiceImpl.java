...
//	private static final Logger logger = Logger.getLogger(ProductService.class);
	...
	public ProductServiceImpl(ProductDAO productDAO, 
			...
			ProductSubOptionService productSubOptionService,
		...
		this.productSubOptionService = productSubOptionService;
	...
	private List<ProductVO> convertToProductVOList(List<ProductTO> productList/*, Map<Long, LinkedList<ImageLinkVO>> imageMap*/) {
		...
		if(productList != null && productList.size() > 0){
			for(ProductTO to : productList){
				...
				//set tags
				vo.setTags(ProductTagsServiceImpl.convertToProductTagsVOList(to.getProductTagsTOList(), vo.getProductId()));
				
				//set image
				LinkedList<ImageLinkVO> imageList = ImageServiceImpl.convertToNewImageLinkVOMapOrdered(to.getImageLinkRsTOList());
				...
				if(subOptionTOList != null && !subOptionTOList.isEmpty()){
					for(ProductSubOptionTO subOptionTO : subOptionTOList){
						List<ProductSubOptionVO> subOptionList = productOptionService.convertToProductSubOptionVOList(Arrays.asList(subOptionTO));
						if(subOptionTO.getProductOptionTO() != null){
							List<ProductOptionVO> optionList = productOptionService.convertToProductOptionVOList(Arrays.asList(subOptionTO.getProductOptionTO()));
	...
	public ImageLinkVO getCoverImageByProductCode(String productCode) {
		...
		if(productTO != null){
			LinkedList<ImageLinkVO> list = ImageServiceImpl.convertToNewImageLinkVOMapOrdered(productTO.getImageLinkRsTOList());
	...
	public void saveAddProduct(ProductVO newProductVO){
		...
		//product info
		productSpecificationService.setProductSpecification(newProductVO.getProductInfo(), newProductTO);
		
		//image
		newProductVO.setImagesLink(ImageServiceImpl.convertFileMetaVOListToImageLinkVOList(newProductVO.getImages(), GeneralUtils.TYPE_PRODUCT));
		imageService.setProductImage(newProductVO.getImagesLink(), newProductTO);
		
		//proudct suboption
		productSubOptionService.setProductSubOption(newProductVO.getOptionList(), newProductTO);
		
		//product attribute
		productAttributeService.setProductAttribute(newProductVO.getProductAttributeVOList(), newProductTO);
