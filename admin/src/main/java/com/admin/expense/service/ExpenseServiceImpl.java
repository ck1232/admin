...
public class ExpenseServiceImpl implements ExpenseService{
	...
	@Override
	public List<ExpenseVO> getAllBadDebtExpense(Date dateAsOf, Date endDate) {
		List<InvoiceTO> invoiceList = invoiceDAO.findByInvoiceDateBetweenAndStatusAndDeleteInd(dateAsOf, endDate, GeneralUtils.STATUS_BAD_DEBT, GeneralUtils.NOT_DELETED);
		return convertToExpenseVO(invoiceList);
	}

	private List<ExpenseVO> convertToExpenseVO(List<InvoiceTO> list) {
		List<ExpenseVO> expenseList = new ArrayList<ExpenseVO>();
		if(list != null && !list.isEmpty()){
			for(InvoiceTO dbObj : list){
				ExpenseVO vo = new ExpenseVO();
	...
	@Override
	public List<ExpenseVO> getAllExpenseExcludeParamType(Date dateAsOf, Date endDate, List<Long> typeList) {
		List<ExpenseTO> expenseList = expenseDAO.findByExpenseDateBetweenAndExpenseTypeIdNotInAndDeleteIndOrderByExpenseDate(dateAsOf, endDate, typeList, GeneralUtils.NOT_DELETED);
		return convertToExpenseVOList(expenseList);
	}

	@Override
	public List<ExpenseVO> getAllExpense(Date dateAsOf, Date endDate, Long expenseTypeId) {
		List<ExpenseTO> expenseList = expenseDAO.findByExpenseDateBetweenAndExpenseTypeIdAndDeleteIndOrderByExpenseDate(dateAsOf, endDate, expenseTypeId, GeneralUtils.NOT_DELETED);
		return convertToExpenseVOList(expenseList);
	}

	@Override
	public List<ExpenseVO> getAllExpenseOfParamType(List<Long> typeList) {
		List<ExpenseTO> expenseList = expenseDAO.findByExpenseTypeIdInAndDeleteIndOrderByExpenseDate(typeList, GeneralUtils.NOT_DELETED);
		return convertToExpenseVOList(expenseList);
	}
